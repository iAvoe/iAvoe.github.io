# 压制教程地图

## x264 x265 AV1 视频压制相关工具下载

用于检查并补完视频编码与播放所需的软件工具

- **主页**：[NazoRip](https://nazorip.site/archives/1482/)
    - *[GitHub 项目页](https://github.com/iAvoe/encoding-tools-download-tutorial)*
    - *[提问与反馈](https://github.com/iAvoe/encoding-tools-download-tutorial/issues)*

---

## 完整版压制教程

用于理解编码原理和参数配置策略。x264 编码教程完整版是新手特别定制款，专门用来系统性地科普基本概念。

1.  **入门**——[x264 教程网页完整版](https://iavoe.github.io/x264-web-tutorial/HTML/index.html)
    - *[GitHub 项目页](https://github.com/iAvoe/x264-web-tutorial)*
    - *[提问与反馈](https://github.com/iAvoe/x264-web-tutorial/issues)*
2.  **进阶**——[x265 教程网页完整版](https://iavoe.github.io/x265-web-tutorial/HTML/index.html)
    - *[GitHub 项目页](https://github.com/iAvoe/x265-web-tutorial)*
    - *[提问与反馈](https://github.com/iAvoe/x265-web-tutorial/issues)*
3.  **进阶**——[AV1 教程网页完整版](https://iavoe.github.io/av1-web-tutorial/HTML/index.html)
    - *[GitHub 项目页](https://github.com/iAvoe/av1-web-tutorial)*
    - *[提问与反馈](https://github.com/iAvoe/av1-web-tutorial/issues)*

---

## 急用版压制教程

最基础知识 + 几套简单易用的预制参数可以套用。其中（lib）x264 教程附带了 OBS 录屏/直播软件的配置。

### x264、x265、libx264、libx265 教程急用版主页

- **主页**：[NazoRip](https://nazorip.site/archives/334)
    - *提问与反馈：教程页留言即可*
- *[GitHub 项目页](https://github.com/iAvoe/x264-x265-copypaste-tutorial-with-obs-record-setting)*
    - *[提问与反馈](https://github.com/iAvoe/x264-x265-copypaste-tutorial-with-obs-record-setting/issues)*

### SVT-AV1 与 libsvtav1 教程急用版主页

- **主页**：[NazoRip](https://nazorip.site/archives/1476)
    - *提问与反馈：教程页留言即可*
- *[GitHub 项目页](https://github.com/iAvoe/x264-x265-copypaste-tutorial-with-obs-record-setting)*
    - *[提问与反馈](https://github.com/iAvoe/x264-x265-copypaste-tutorial-with-obs-record-setting/issues)*

### 网盘存档

- [谷歌盘（drive.google.com）](https://drive.google.com/drive/folders/1kFCeNGA_wiiLt-DSeI3cyY8vxlffgQcy?usp=sharing)
- [百度云（pan.baidu.com）](https://pan.baidu.com/s/1sbz8WztGTz3lcLzirHW_2w)
- [QQ 群 691892901 的群文件](https://jq.qq.com/?_wv=1027&k=5YJFXyf)

---

## PowerShell 批处理智能压制工具（bbenc）

一款容易上手的强力效率工具。主打自动根据视频规格，用户需求，电脑硬件等条件，给出 ffmpeg、vspipe（API 自适应）、avs2yuv（AviSynth(+) 自适应）、avs2pipemod、SVFI 源，到 x264、x265、SVT-AV1 编码器参数的定制。附带 VS/AVS 脚本自动生成功能，以及复杂封装命令生成工具。

- **主页**：[NazoRip](https://nazorip.site/archives/1101/)
    - *[GitHub 项目页](https://github.com/iAvoe/Batch-batch-encoder/tree/main)*
    - *[提问与反馈](https://github.com/iAvoe/Batch-batch-encoder/issues)*

---

## 其它相关教程

1. **打理**——[mpv 播放器的安装与设置](https://nazorip.site/archives/1052/)
    - *提问与反馈：教程页留言即可*
2. **入门**——[QAAC 音频压缩与转码教程](https://www.nazorip.site/archives/44/) 或 [GitHub 副本](https://github.com/iAvoe/QAAC-Tutorial-Standalone/blob/master/%E6%95%99%E7%A8%8B.md)
    - *提问与反馈：教程页留言即可*
    - *[GitHub 提问与反馈](https://github.com/iAvoe/QAAC-Tutorial-Standalone/issues)*
3. **进阶**——[ffprobe 教程](https://nazorip.site/archives/169/) 或 [GitHub 副本](https://github.com/iAvoe/FFprobe-Tutorial-Standalone/blob/master/%E6%95%99%E7%A8%8B.md)
    - *提问与反馈：教程页留言即可*
    - *[GitHub 提问与反馈](https://github.com/iAvoe/FFprobe-Tutorial-Standalone/issues)*
4. **进阶**——[ffprobe + Excel 绘制码率曲线，帧类型饼图](https://nazorip.site/archives/1068)
    - *提问与反馈：教程页留言即可*
5. **进阶**——[使用 Excel 绘制逐帧 VMAF 画质图表的数据可视化教程](https://nazorip.site/archives/1458)
    - *提问与反馈：教程页留言即可*
6. **进阶**——[交错转逐行与 IVTC 处理](https://iavoe.github.io/deint-ivtc-web-tutorial/HTML/index.html)
    - *[GitHub 项目页](https://github.com/iAvoe/deint-ivtc-web-tutorial)*
    - *[提问与反馈](https://github.com/iAvoe/deint-ivtc-web-tutorial/issues)*
7. **进阶**——[图像序列压制教程](https://iavoe.github.io/img-sequence-enc-web-tutorial/HTML/index.html)
    - *[GitHub 项目页](https://github.com/iAvoe/img-sequence-enc-web-tutorial)*
    - *[提问与反馈](https://github.com/iAvoe/img-sequence-enc-web-tutorial/issues)*